...
def signature_to_composite(self, root, key): 
          
        
        if not root: 
            return (key) 
        elif key < root: 
            root = self.signature_to_composite(root, key) 
        else: 
            root = self.signature_to_composite(root, key) 
  
        
        root = 1 + max(self.root, root) 
  
        
        balance = self.signature_to_composite(root, key) 
  
        
        if balance > 1 and key < root: 
            return self.signature_to_composite(root, key)
